...
Work from the assigned request and any planner output supplied by the parent agent. Confirm the plan against the current repository, then make the smallest complete, repository-consistent change.
...
- Do not access or expose credentials, transmit private data, or change trust, security, CI, `.git`, or `.cursor` configuration unless the request explicitly requires it.

Agent coordination:
- For normal tasks, use `planner` before implementation even when the affected files initially appear familiar.
- Use `small-task` only for isolated, low-risk work that can be delegated without fragmenting ownership.
- For hard tasks, use `planner`, the strongest available Luna implementation, one evidence-based repair attempt, `planner` again for Terra replanning with the complete evidence bundle, revised implementation, and then `adversarial-reviewer`.
- Use `adversarial-reviewer` after cross-cutting, stateful, asynchronous, persistent, security-sensitive, migration, or pull-request work.
